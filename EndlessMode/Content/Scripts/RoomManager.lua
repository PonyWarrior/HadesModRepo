...
function ValidateIdLeaks( trace, tableToCheck )
	...
	local idLeakIgnores = ToLookup({ "InspectPoints", "RunHistory", "RoomHistory", "Hero", "PostLineFunctionArgs", })
...
function DoPatches()
	...
	if GameState ~= nil then
		...
		GameState.EncountersOccuredCache = GameState.EncountersOccuredCache or {}
		...
		if GameState.Gift.NPC_Achilles_01 ~= nil and GameState.Gift.NPC_Achilles_01.Value > 6 and not TextLinesRecord.AchillesGift07_A then
		...
		if GameState.Gift.NPC_Thanatos_01 ~= nil and GameState.Gift.NPC_Thanatos_01.Value > 6 and not TextLinesRecord.ThanatosGift07_A then
	...
	if CurrentRun ~= nil then
		...
		if CurrentRun.CurrentRoom ~= nil then
			...
			CurrentRun.CurrentRoom.UnthreadedEvents = RoomData[CurrentRun.CurrentRoom.Name].UnthreadedEvents
			...
			if CurrentRun.CurrentRoom.Name == "C_Boss01" then
				CurrentRun.CurrentRoom.UnthreadedEvents = RoomData.C_Boss01.UnthreadedEvents
			...
			if CurrentRun.CurrentRoom.UnthreadedEvents ~= nil then
				for k, event in pairs( CurrentRun.CurrentRoom.UnthreadedEvents ) do
					if event.FunctionName == "BossIntro" and event.Args ~= nil and event.Args.SetupBossIds == nil then
						CurrentRun.CurrentRoom.UnthreadedEvents = RoomData[CurrentRun.CurrentRoom.Name].UnthreadedEvents
					end
				end
			end
			...
			if CurrentRun.CurrentRoom.Name == "D_Reprieve01" then
				CurrentRun.CurrentRoom.StartUnthreadedEvents = RoomData.D_Reprieve01.StartUnthreadedEvents
			end
		...
		if CurrentRun.Hero ~= nil then
			...
			for i, rarityTraitData in pairs(rarityTraits) do
				if rarityTraitData.ReplaceBonus then
					CurrentRun.Hero.BoonData.ReplaceChance = CurrentRun.Hero.BoonData.ReplaceChance + rarityTraitData.ReplaceBonus
				end
			end
			--/
			...
			if not Contains(CurrentRun.Hero.Binks, "ZagreusInteractionFishing_Bink") then
				table.insert(CurrentRun.Hero.Binks, "ZagreusInteractionFishing_Bink")
			end
			...
			for i, trait in pairs(CurrentRun.Hero.Traits) do
				...
				if trait.PropertyChanges ~= nil then
					for k, propertyChange in pairs( trait.PropertyChanges ) do
						if propertyChange.WeaponProperty == "AdditionalProjectileDamageMultiplier" then
							addTraitToUpdate( trait )
...
function ValidateMaxHealth()
	...
	if expectedMaxHealth ~= CurrentRun.Hero.MaxHealth then
		...
		CurrentRun.Hero.MaxHealth = CurrentRun.Hero.MaxHealth + delta
		CurrentRun.Hero.Health = CurrentRun.Hero.Health + delta
	end
end
...
function SetupMap()
	...
	CreateGroup({ Name = "Vignette", BlendMode = "Overlay" })
	...
	InsertGroupInFront({ Name = "Combat_Menu", DestinationName = "Combat_UI" })
	...
	InsertGroupInFront({ Name = "Combat_Menu_Overlay", DestinationName = "Combat_Menu_Additive" })
...
function AttemptUseDoor( door )
	...
	if CurrentRun.CurrentRoom.SellTraitShrineUpgrade and CurrentRun.CurrentRoom.SellTraitShop ~= nil and GetNumMetaUpgrades("ForceSellShrineUpgrade") > 0 and not CurrentRun.CurrentRoom.SoldTrait and TableLength(CurrentRun.CurrentRoom.SellOptions) > 0 then
		...
		if CheckCooldown( "DoorLocked", 1.0 ) then
			...
			thread( InCombatText, CurrentRun.Hero.ObjectId, "ExitBlockedBySellWell", 1.5 )
...
function StartRoom( currentRun, currentRoom )
	...
	if GetNumMetaUpgrades("BiomeSpeedShrineUpgrade") > 0 then
		...
		if previousArea == nil or ( currentArea ~= previousArea and Contains(BiomeTimeLimits.ValidBiomes, currentArea ) and Contains(BiomeTimeLimits.ValidBiomes, previousArea )) then
			CurrentRun.BiomeTime = BiomeTimeLimits.Timers[GetNumMetaUpgrades("BiomeSpeedShrineUpgrade")][currentAreaIndex]
		end
	...
	if startTriggerDistance ~= nil and startTriggerDistance > 0 then
		...
		if not IsEmpty( encounterStartIds ) then
			...
			NotifyWithinDistanceAny({ Id = currentRun.Hero.ObjectId, DestinationIds = encounterStartIds, Distance = startTriggerDistance, Notify = notifyName })
...
function RestoreUnlockRoomExits( currentRun, currentRoom )
	...
	Destroy({ Name = "WaterEndingObjectsDestroy" })
...
function LoadSpawnPackages( encounter )
	...
	for _, packageName in pairs(GetHeroTraitValues("LoadPackages")) do
...
function GetEncounterBinks( currentEncounter, binksToPreload, preloadedEnemyTypes )
	...
	if currentEncounter.EnemySet ~= nil then
		-- Due to challenge encounters, all Biome enemies must currently be loaded at all times
...
function StartEncounter( currentRun, currentRoom, currentEncounter )
	...
	if CurrentRun.Hero.Health / CurrentRun.Hero.MaxHealth <= HealthUI.LowHealthThreshold then
	...
	if currentRoom.PendingCodexUpdate and currentEncounter == currentRoom.Encounter then
...
function GiveRandomConsumables( args )
	...
	for i, lootData in pairs(args.LootOptions) do
		...
		elseif lootData.MinAmount ~= nil and lootData.MaxAmount ~= nil then
			local amount = RandomInt(lootData.MinAmount, lootData.MaxAmount)
			if lootData.Name == "Money" then
				...
				for i = 1, amount do
					...
					ApplyUpwardForce({ Id = consumableId, Speed = RandomFloat( 500, 700 ) })
					ApplyForce({ Id = consumableId, Speed = RandomFloat( 75, 150 ), Angle = RandomFloat( 0, 360 ), SelfApplied = true })
					SetObstacleProperty({ Property = "MagnetismWhileBlocked", Value = 0, DestinationId = consumable.ObjectId })
				end
			end
		end
...
function CreateLoot( args )
	...
	if args.AddBoostedAnimation then
		CreateAnimation({ DestinationId = loot.ObjectId, Name = "RoomRewardBoosted" })
	end
...
function CheckAmmoDrop( currentRun, targetId, ammoDropData, numDrops )
	...
	if ammoDropData.Count == nil or ammoDropData.Count <= 0 then
		return
	end
...
function CheckTreasureImpSpawn(currentRun, currentRoom)
	local ammoIds = GetIdsByType({ Name = "AmmoPack" })
	local ammoCount = TableLength(ammoIds)
	local ammoCountRequirement = EnemyData.TreasureImp.AmmoCountRequirement or 3

	if not IsGameStateEligible(currentRun, EnemyData.TreasureImp) then
		return
	end

	if ammoCount < ammoCountRequirement then
		return
	end

	if currentRoom.SpawnedTreasureImp then
		return
	end

	if currentRoom.BlockTreasureImps then
		return
	end

	local spawnChance = EnemyData.TreasureImp.SpawnChanceOnAmmoDrop or 0.5

	-- Spawn Treasure Imp
	local newUnit = DeepCopyTable( EnemyData.TreasureImp )
	local spawnPointIds = GetIds({Name = "SpawnPoints"})
	local spawnPointId = GetClosest({ Id = CurrentRun.Hero.ObjectId, DestinationIds = spawnPointIds, Distance = 1000 })
	if spawnPointId ~= nil then
		newUnit.ObjectId = SpawnUnit({ Name = "TreasureImp", Group = "Standing", DestinationId = spawnPointId })
		SetupEnemyObject( newUnit, CurrentRun)
		currentRoom.SpawnedTreasureImp = true
		thread( InCombatText, newUnit.ObjectId, "TreasureImpEngage", 4 )
	end
end

function CalculateChallengeMoneyDrop( currentRoom, challengeEncounter )
	local moneyDropData = challengeEncounter.MoneyDropData
	local runDepth = GetRunDepth( CurrentRun )
	local baseValue = moneyDropData.BaseValue + runDepth
	local amountVariance = RandomFloat(0.8, 1.2)
	local amount = baseValue * amountVariance

	if moneyDropData.ValueMultiplier then
		amount = amount * moneyDropData.ValueMultiplier
	end
	amount = round(amount)
	return amount
end
...
function DropMoney( amount, args )
	...
	PlaySound({ Name = "/Leftovers/Menu Sounds/CoinFlash", Id = moneyDropId })
end
...
function AddMoney( amount, source )
	...
	if HasHeroTraitValue("BlockMoney") then
		amount = 0
		PlaySound({ Name = "/SFX/Player Sounds/DarknessCollectionPickupSMALL" })
	else
...
function OnGiftPointsAdded( name, amount, source, args )
	...
	if not HasResource( "GiftPoints" ) then
...
function IsCombatEncounterActive( currentRun )
	...
	if not currentRun.Hero.IsDead then
		...
		if currentRun.CurrentRoom.Encounter ~= nil and currentRun.CurrentRoom.Encounter.EncounterType == "NonCombat" then
			return false
		end
		...
		if currentRun.CurrentRoom.Encounter ~= nil and not currentRun.CurrentRoom.Encounter.InProgress then
			return false
		end
	end
	return false
...
function GetNextSpawn( currentEncounter )
	...
	for k, spawnInfo in orderedPairs( currentEncounter.Spawns ) do
		if spawnInfo.InfiniteSpawns or spawnInfo.RemainingSpawns > 0 then
			local enemyData = EnemyData[spawnInfo.Name]
			if enemyData ~= nil and enemyData.MaxActiveCount ~= nil then
				local enemyCount = 0
				-- @optimization Convert to buckets by type
				for enemyId, enemy in pairs( ActiveEnemies ) do
					if enemy.Name == enemyData.Name then
						enemyCount = enemyCount + 1
					end
				end
				if enemyCount < enemyData.MaxActiveCount then
	...
	local randomSpawnInfo = GetRandomValue( remainingSpawnInfo )
...
function SetupEnemyObject( newEnemy, currentRun, args )
	...
	--Endless mode
	if CurrentRun.Hero.EndlessModLoopedTimes ~= nil and CurrentRun.Hero.EndlessModLoopedTimes > 0 then
		healthMultiplier = 1 + ( healthMultiplier * ( (CurrentRun.Hero.EndlessModLoopedTimes + 1.0) ) )
	end
	--/
	...
	if newEnemy.UseShrineUpgrades then
		...
		--Endless Mode
		if CurrentRun.Hero.EndlessModLoopedTimes ~= nil and CurrentRun.Hero.EndlessModLoopedTimes > 0 then
			speedMultiplier = speedMultiplier * ( ( ( CurrentRun.Hero.EndlessModLoopedTimes + 1.0 ) / 10.0 ) + 1.0)
		end
		--/
	...
	if newEnemy.MaskHue and newEnemy.MaskHue2 then
		SetPlayerUnitProperty({ DestinationId = newEnemy.ObjectId, Name = "MaskHue", Value = newEnemy.MaskHue })
		SetPlayerUnitProperty({ DestinationId = newEnemy.ObjectId, Name = "MaskHue2", Value = newEnemy.MaskHue2 })
		SetPlayerUnitProperty({ DestinationId = newEnemy.ObjectId, Name = "MaskSaturationAddition", Value = newEnemy.MaskSaturationAddition })
		SetPlayerUnitProperty({ DestinationId = newEnemy.ObjectId, Name = "MaskSaturationAddition2", Value = newEnemy.MaskSaturationAddition2 })
		SetPlayerUnitProperty({ DestinationId = newEnemy.ObjectId, Name = "MaskValueAddition", Value = newEnemy.MaskValueAddition })
		SetPlayerUnitProperty({ DestinationId = newEnemy.ObjectId, Name = "MaskValueAddition2", Value = newEnemy.MaskValueAddition2 })
	end
...
function SetAvailableUseText( source )
	...
	local canGift = ( source.FallbackGiftText ~= nil ) or ( GetTextLinesUseableStatus( source, source.GiftTextLineSets ) and CanReceiveGift( source ) and ( HasResource( GetNextGiftResource( genusName ), GetNextGiftResourceQuantity( genusName ))))
	if source.NextInteractLines ~= nil or canGift or source.AlwaysShowDefaultUseText then
...
function SetNextInteractLines( source, textLines )
	...
	if textLines.AngleTowardTargetId ~= nil then
		AngleTowardTarget({ Id = source.ObjectId, DestinationId = textLines.AngleTowardTargetId })
	end
...
function UnlockRoomExits( run, room, delay )
	...
	if room.Encounter ~= nil and not room.Encounter.SkipExitReadyCheckpoint then
		...
		SetConfigOption({ Name = "ValidCheckpoint", Value = true })
		SaveProfile({ ShowSaveSpinner = true })
...
function DoUnlockRoomExits( run, room )
	...
	if CurrentRun.CurrentRoom.ForceFishing and CurrentRun.CurrentRoom.FishingPointId then
		PlaySound({ Name = "/Leftovers/SFX/AnnouncementPing7", Id = CurrentRun.CurrentRoom.FishingPointId, Delay = 1.0 })
		SetAnimation({ Name = "FishingPointActive", DestinationId = CurrentRun.CurrentRoom.FishingPointId })
	end
...
function RecordEncounter( run, encounter )
	...
	run.EncountersOccuredCache[encounter.Name] = (run.EncountersOccuredCache[encounter.Name] or 0) + 1
	GameState.EncountersOccuredCache[encounter.Name] = (GameState.EncountersOccuredCache[encounter.Name] or 0) + 1

	run.EncountersOccuredBiomeCache[encounter.Name] = (run.EncountersOccuredBiomeCache[encounter.Name] or 0) + 1
...
function CheckDistanceTrigger( trigger, triggerSource, id )
	...
	while not triggeredOnce or trigger.Repeat do
		...
		if trigger.WithinDistance ~= nil then
			...
			NotifyWithinDistanceAny({ Id = currentRun.Hero.ObjectId, DestinationNames = trigger.TriggerGroup, DestinationIds = triggerIds, Distance = trigger.WithinDistance, ScaleY = trigger.ScaleY or 1.0, Notify = notifyName })
			waitUntil( notifyName )
			notifiedById = NotifyResultsTable[notifyName]
		end
		...
		if trigger.DeleteGroup ~= nil then
			Destroy({ Name = trigger.DeleteGroup })
		end
		...
		if trigger.Emote ~= nil then
			PlayEmote( triggerId, trigger.Emote, triggerSource.EmoteOffsetZ )
		end
		...
		if trigger.Repeat then
			...
			if trigger.OutsideDistance ~= nil then
				...
				NotifyWithinDistanceAny({ Id = currentRun.Hero.ObjectId, DestinationNames = trigger.TriggerGroup, DestinationIds = triggerIds, Distance = trigger.OutsideDistance - repeatBufferDistance, Notify = notifyName })
				waitUntil( notifyName )
			end
			if trigger.WithinDistance ~= nil then
		...
		elseif trigger.LeaveDistanceBuffer ~= nil then
			...
			if trigger.OutsideDistance ~= nil then
				...
				NotifyWithinDistanceAny({ Id = currentRun.Hero.ObjectId, DestinationNames = trigger.TriggerGroup, DestinationIds = triggerIds, Distance = trigger.OutsideDistance - trigger.LeaveDistanceBuffer, Notify = notifyName })
				waitUntil( notifyName )
			end
			if trigger.WithinDistance ~= nil then
				local notifyName = "WithinDistanceRepeatBuffer"..(trigger.TriggerGroup or trigger.TriggerObjectType)
				NotifyOutsideDistanceAll({ Id = currentRun.Hero.ObjectId, DestinationNames = trigger.TriggerGroup, DestinationIds = triggerIds, Distance = trigger.WithinDistance + trigger.LeaveDistanceBuffer, Notify = notifyName })
				waitUntil( notifyName )
 			end
...
function HandleSecretSpawns( currentRun )
	...
	if not IsEmpty( challengeBaseIds ) and IsChallengeSwitchEligible( currentRun, TableLength( challengeBaseIds )) then
		...
		for k, challengeName in pairs( EncounterSets.ChallengeOptions ) do
			...
			if hasForceTrait or challengeData.Requirements == nil or IsGameStateEligible( CurrentRun, challengeData, challengeData.Requirements ) then
	...
	for enemyId, enemy in pairs( ActiveEnemies ) do
		if enemy.ToggleTrap then
			...
			if enemy.DisabledAnimation ~= nil then
				SetAnimation({ DestinationId = enemyId, Name = enemy.DisabledAnimation })
			end
