function CreateNewHero( prevRun, args )
	...
	newHero.EndlessModLoopedTimes = 0
...
function StartNewGame()
	...
	GameState.EncountersOccuredCache = {}
...
function IsRoomForced( currentRun, currentRoom, nextRoomData, args )
	...
	if nextRoomData.ForceAtWingDepthMin ~= nil and currentRun.WingDepth >= nextRoomData.ForceAtWingDepthMin then
		if currentRun.WingDepth >= nextRoomData.ForceAtWingDepthMax then
			return true
		else
			local forcedChance = 1 / (nextRoomData.ForceAtWingDepthMax - currentRun.WingDepth)
			if RandomChance( forcedChance ) then
				return true
			end
		end
...
function IsRoomEligible( currentRun, currentRoom, nextRoomData, args )
	...
	for k, mutator in pairs( GameState.ActiveMutators ) do
		if mutator.RemoveRooms ~= nil then
			if Contains( mutator.RemoveRooms, nextRoomData.Name ) then
				return false
			end
		end
	end
	...
	if not IsGameStateEligible( currentRun, nextRoomData , nil, args) then
		return false
	end
	return true
...
function GenerateEncounter( currentRun, room, encounter )
	...
	--Endless Mode
	if CurrentRun.Hero.EndlessModLoopedTimes > 0.0 then
		depthMultiplier = depthMultiplier * (CurrentRun.Hero.EndlessModLoopedTimes + 1.0)
	end
	--/
				...
				table.insert(encounter.DistanceTriggers,
					{
						...
						TriggerGroup = {"GroundEnemies", "FlyingEnemies"}, WithinDistance = 700,
...
function IsEncounterEligible( currentRun, room, nextEncounterData )
	...
	if nextEncounterData.MaxAppearancesThisRun ~= nil and currentRun.EncountersOccuredCache[nextEncounterData.Name] ~= nil and currentRun.EncountersOccuredCache[nextEncounterData.Name] >= nextEncounterData.MaxAppearancesThisRun then
		return false
	end
	if nextEncounterData.MaxAppearancesThisBiome ~= nil and currentRun.EncountersOccuredBiomeCache[nextEncounterData.Name] ~= nil and currentRun.EncountersOccuredBiomeCache[nextEncounterData.Name] >= nextEncounterData.MaxAppearancesThisBiome then
		return false
	end
	...
	if nextEncounterData.MinRunsSinceThanatosSpawn ~= nil then
		...
		for runIndex = #GameState.RunHistory + 1, 1, -1 do
			...
			if prevRun.ThanatosSpawns ~= nil and prevRun.ThanatosSpawns > 0 then
				if runsSinceOccured < nextEncounterData.MinRunsSinceThanatosSpawn then
					--DebugPrint({ Text = "runsSinceOccured = "..runsSinceOccured })
					return false
				end
			end
...
function HasEncounterOccurred( currentRun, encounterName, requireCompleted )
	...
	if requireCompleted then
		...
		if currentRun.EncountersOccuredCache[encounterName] ~= nil then
			return true
		end
	end
...
function IsRoomRewardEligible( run, room, reward, previouslyChosenRewards, args )
	...
	for k, mutator in pairs( GameState.ActiveMutators ) do
		if mutator.RemoveRoomRewards ~= nil then
			if Contains( mutator.RemoveRoomRewards, reward.Name ) then
				return false
			end
		end
	end
...
function UpdateRunHistoryCache( run, roomAdded )
	...
	run.EncountersOccuredCache = run.EncountersOccuredCache or {}
	run.EncountersOccuredBiomeCache = run.EncountersOccuredBiomeCache or {}
	...
	if roomAdded ~= nil then
		...
		if roomAdded.RoomSetName ~= run.RoomHistory[#run.RoomHistory].RoomSetName then
			...
			run.EncountersOccuredBiomedCache = {}
		end
...
function GetRunDepth( currentRun )
	--Endless Mode
	if currentRun.RoomHistory == nil and CurrentRun.Hero.EndlessModLoopedTimes < 1 then
		return 1
	end
...
function GetBiomeRoomHistory( currentRun )
	...
	for roomIndex = #currentRun.RoomHistory, 1, -1 do
		...
		table.insert( roomHistory, 1, room )
	end
	return roomHistory
...
function GetNumRunsClearedWithWeapon( weaponName )
	...
	for k, prevRun in pairs( GameState.RunHistory ) do
		if prevRun.WeaponsCache ~= nil and prevRun.WeaponsCache[weaponName] then
			if IsSameMode( prevRun ) and prevRun.Cleared then
				runsCleared = runsCleared + 1
			end
		end
	end
...
function RecordRunCleared()
	...
	for k, weaponName in pairs( WeaponSets.HeroMeleeWeapons ) do
...
function GetHighestRunClearShrinePointThreshold()
	...
	for i, weaponName in pairs( WeaponSets.HeroMeleeWeapons ) do
...
function GetHighestShrinePointRunClear( currentRun )
...
function IsGameStateEligible( currentRun, source, requirements, args )
	...
	if requirements.ForceAtRunDepth ~= nil and currentRunDepth ~= requirements.ForceAtRunDepth then
		return false
	end
	if requirements.ForceAtRunDepthMin ~= nil and currentRunDepth < requirements.ForceAtRunDepthMin then
		return false
	end
	...
	if requirements.ForceAtBiomeDepth ~= nil and currentBiomeDepth ~= requirements.ForceAtBiomeDepth then
		return false
	end
	if requirements.ForceAtBiomeDepthMin ~= nil and currentBiomeDepth < requirements.ForceAtBiomeDepthMin then
		return false
	end
	if requirements.ForceAtWingDepthMin ~= nil and currentRun.WingDepth < requirements.ForceAtWingDepthMin then
		return false
	end
	...
	if requirements.RequiredCodexEntriesMin ~= nil then
		if CalcNumCodexEntiresUnlocked() < requirements.RequiredCodexEntriesMin then
			return false
		end
	end
	...
	if requirements.ConsecutiveDeathsInRoom ~= nil then
		...
		if HasSeenRoomInRun( currentRun, requirements.ConsecutiveDeathsInRoom.Name ) then
		...
		DebugPrint({ Text = "consecutiveDeathsInRoom = "..consecutiveDeathsInRoom })
	...
	if requirements.ConsecutiveClearsOfRoom ~= nil then
		...
		DebugPrint({ Text = "consecutiveClearsOfRoom = "..consecutiveClearsOfRoom })
	...
	if requirements.RequiresInOnslaught then
		if not GameState.ActiveOnslaught then
			return false
		end
	end

	if requirements.RequiresNotInOnslaught then
		if GameState.ActiveOnslaught then
			return false
		end
	end
	...
	if requirements.RequirePreviousRoomSet ~= nil then
		for roomIndex = #currentRun.RoomHistory, 1, -1 do
			if not currentRun.RoomHistory[roomIndex].UsePreviousRoomSet then
				if currentRun.RoomHistory[roomIndex].RoomSetName ~= requirements.RequirePreviousRoomSet then
					return false
				end
				break
			end
	...
	if requirements.RequiredAnyEncountersThisRun ~= nil then
		...
		for k, encounterName in pairs( requirements.RequiredAnyEncountersThisRun ) do
			if currentRun.EncountersOccuredCache[encounterName] ~= nil and currentRun.EncountersOccuredCache[encounterName] >= 1 then
				anyTrue = true
				break
			end
		end
	...
	if requirements.RequiredCompletedRunsModulo ~= nil then
		local remainder = GetCompletedRuns() % requirements.RequiredCompletedRunsModulo
		if remainder ~= requirements.RequiredCompletedRunsModuloRemainder then
			return false
		end
	end
	...
	if requirements.RequiredClearsWithWeapons ~= nil then
		for k, weaponName in pairs( requirements.RequiredClearsWithWeapons ) do
			if GameState.TimesClearedWeapon == nil or GameState.TimesClearedWeapon[weaponName] == nil or GameState.TimesClearedWeapon[weaponName] <= 0 then
				return false
			end
		end
	end
	...
	if requirements.RequiredOnlyNotPickedUp ~= nil then
		...
		for i, lootName in pairs(eligibleLootNames) do
			local lootData = LootData[lootName]
			if not lootData.DebugOnly and lootData.GodLoot and not GameState.LootPickups[lootName] and lootName ~= requirements.RequiredOnlyNotPickedUp then
				return false
			end
		end
	end
	...
	if requirements.RequiredObjectId ~= nil and requirements.ObjectId ~= requirements.RequiredObjectId then
		return false
	end
	...
	if requirements.AnyAffordableGhostAdminItem ~= nil then
		...
		for itemName, itemData in pairs( ConditionalItemData ) do
			if not itemData.DebugOnly and itemData.ResourceCost ~= nil and not GameState.CosmeticsAdded[itemName] then
	...
	if requirements.RequiredMinCaughtFish ~= nil and GetNumHeldFish() < requirements.RequiredMinCaughtFish then
		return false
	end
	...
	if requirements.MinRunsSinceSquelchedHermes ~= nil then
		...
		for runIndex = #GameState.RunHistory + 1, 1, -1 do
			...
			if prevRun.SquelchedHermes then
				if runsSinceOccured < requirements.MinRunsSinceSquelchedHermes then
					--DebugPrint({ Text = "runsSinceOccured = "..runsSinceOccured })
					return false
				end
			end
	...
	if requirements.MaxRunsSinceSquelchedHermes ~= nil then
		...
		for runIndex = #GameState.RunHistory + 1, 1, -1 do
			...
			if prevRun.SquelchedHermes then
				...
				if runsSinceOccured >= requirements.MaxRunsSinceSquelchedHermes then
					--DebugPrint({ Text = "runsSinceOccured = "..runsSinceOccured })
					return false
				end
			end
	...
	if requirements.MutatorActive ~= nil then
		local found = false
		for k, mutator in pairs( GameState.ActiveMutators ) do
			if mutator.Name == requirements.MutatorActive then
				found = true
				break
			end
		end
	...
	if requirements.MutatorInactive ~= nil then
		for k, mutator in pairs( GameState.ActiveMutators ) do
			if mutator.Name == requirements.MutatorActive then
				return false
			end
	...
	if requirements.RequiredInteractionThisRun ~= nil then
		if not CurrentRun.NPCInteractions[requirements.RequiredInteractionThisRun] then
			return false
		end
	end
	...
	if requirements.RequiredMinDoorsClosedInRoom ~= nil then
		if TableLength( currentRun.ClosedDoors[currentRun.CurrentRoom.Name] or {} ) < requirements.RequiredMinDoorsClosedInRoom then
			return false
		end
	end
	if requirements.RequiredMaxDoorsClosedInRoom ~= nil then
		if TableLength( currentRun.ClosedDoors[currentRun.CurrentRoom.Name] or {} ) > requirements.RequiredMaxDoorsClosedInRoom then
			return false
		end
	end
	...
	if requirements.RequiresNewMusicTracks ~= nil then
		local anyTrue = false
		for k, trackData in pairs( MusicPlayerTrackData ) do
